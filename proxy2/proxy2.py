...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    def connect_intercept(self):
        ...
        with self.lock:
            if not os.path.isfile(certpath):
                ...
                p1 = Popen(["openssl", "req", "-new", "-key", self.certkey, "-subj", "/CN={}".format(hostname)],
                           stdout=PIPE)
    ...
    def do_GET(self):
        if self.path == 'http://proxy2.test/' or self.path == 'http://proxy2.test':
            self.send_cacert()
            return
        ...
        if res_body_modified == 502:
            ...
            # print(res_body_modified.decode())
            res_body_plain = res_body_modified
            res_body = self.encode_content_body(res_body_plain, content_encoding)
            # print(str(res.headers), type(res.headers))
            try:
                res.headers.replace_header('Content-Length', str(len(res_body)))
            except KeyError:
                pass
        setattr(res, 'headers', self.filter_headers(res.headers))
        ...
        if res_body is not None:
            ...
            if content_type.startswith('application/json'):
                ...
                if m:
                    h = HTMLParser()
                    if m.group(1):
                        print_color(32, "==== HTML TITLE ====\n{}\n".format(html.unescape(m.group(1).decode())))
